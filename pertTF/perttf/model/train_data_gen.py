...
def add_pred_layer(adata: AnnData, 
    ...
    ct_key = "celltype_2"
    if ct_key not in adata_p.obs.columns:
        raise ValueError("[add_pred_layer] Required obs column 'celltype_2' not found.")
    adata_p.obs[ct_key] = adata_p.obs[ct_key].astype(str)
    ...
    if ct_key in adata_p.obs.columns:
        celltypes_labels_0 = adata_p.obs[ct_key].astype(str).tolist()  # make sure count from 0
    else:
    ...
    if next_cell_pred == "pert":
        ...
        obsf=adata_p.obs
        next_cell_dict = {}
        for candidate_celltype in set(obsf[ct_key]):
            next_cell_dict[candidate_celltype] = {}
            for candidate_genotype in set(obsf['genotype']):
                obsf_sel = obsf[(obsf[ct_key]==candidate_celltype) & (obsf['genotype']==candidate_genotype)]
        ...
        for this_cell in adata_p.obs.index.values:
            this_cell_type = adata_p.obs.loc[this_cell][ct_key]
...
def produce_training_datasets(adata_input, config,
    ...
    """
#    ct_key = "celltype_2" if "celltype_2" in adata_input.obs.columns else "celltype"
    # --- STRICT: use celltype_2 only ---
    ct_key = "celltype_2"
    if ct_key not in adata_input.obs.columns:
        raise ValueError("[produce_training_datasets] Required obs column 'celltype_2' not found.")
    adata_input.obs[ct_key] = adata_input.obs[ct_key].astype(str)

    if cell_type_to_index is None:
        ct_values = adata_input.obs[ct_key].astype(str)
        cell_type_to_index = {ct: i for i, ct in enumerate(sorted(ct_values.unique()))}
    if genotype_to_index is None:
          geno_values = adata_input.obs["genotype"].astype(str)
          genotype_to_index = {g: i for i, g in enumerate(sorted(geno_values.unique()))}
    ...
    if cell_type_to_index is None:
        cell_type_to_index = {cell_type: index for index, cell_type in enumerate(set(adata_input.obs["celltype_2"].tolist()))}
