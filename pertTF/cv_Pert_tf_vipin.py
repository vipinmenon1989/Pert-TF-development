...
adata0 = sc.read_h5ad("object_integrated_assay3_annotated_final.cleaned.h5ad")
logger.info(f"Loaded AnnData: n_obs={adata0.n_obs}, n_vars={adata0.n_vars}")

# example cleaning similar to original
# ---- use celltype_2 as truth ----
if "celltype_2" not in adata0.obs.columns:
    raise ValueError("celltype_2 not found in adata0.obs. Load the updated h5ad or fix mapping.")

valid = adata0.obs["celltype_2"].notna()
dropped = int((~valid).sum())
adata0 = adata0[valid].copy()
logger.info(f"Dropped {dropped} cells missing celltype_2.")

# keep pipeline compatibility: store labels in 'celltype'
adata0.obs["celltype_2"] = adata0.obs["celltype_2"].astype(str)
adata0.obs["celltype"] = adata0.obs["celltype_2"]
