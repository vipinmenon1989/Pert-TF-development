...
# =========================
BASE_DIR <- "/local/projects-t3/lilab/vmenon/Pert-TF-model/pertTF/Identity_model/metric_bovplot_validation_per_class/"
...
# Filename patterns (matches your listing)
PATTERNS <- list(
...
# One function to build both the combined facet plot and per-metric plots
make_per_class_plots <- function(df, task, out_dir = OUT_DIR, error_type = c("sd","sem")) {
  ...
  # Order classes by mean support (desc) for nicer layout
  class_order <- long %>%
    group_by(class) %>%
  ...
  # ---------- (B) Individual figures per metric (easier for papers) ----------
