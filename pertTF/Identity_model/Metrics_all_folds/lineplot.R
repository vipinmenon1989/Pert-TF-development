...
# CONFIG
#=====================
base_dir <- "/local/projects-t3/lilab/vmenon/Pert-TF-model/pertTF/Identity_model/Metrics_all_folds"
pattern  <- "^curve_metrics_fold_[1-5]+\\.csv$"
...
extract_fold_from_name <- function(fname)
  as.integer(sub("^.*fold_([1-5]+)\\.csv$", "\\1", basename(fname)))
