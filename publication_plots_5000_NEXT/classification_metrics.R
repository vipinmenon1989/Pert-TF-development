...
FAMILIES <- c("celltype_per_class", "genotypeNEXT_per_class")
## =======================================================

dir_create(OUT_DIR)

# --- Filename patterns (genotypeNEXT kept throughout) ---
LIST_RX  <- "^validation_fold[0-9]+_[A-Za-z0-9_]+_(celltype_per_class|genotypeNEXT_per_class)_[A-Za-z0-9_]+\\.csv$"
PARSE_RX <- "^validation_fold(\\d+)_([A-Za-z0-9_]+)_(celltype_per_class|genotypeNEXT_per_class)_([A-Za-z0-9_]+)\\.csv$"
...
plot_one_metric_with_avg <- function(df_long, metric_name, family_label) {
  ...
  # Order classes by Avg descending (fallback: keep input order)
  class_order <- if (nrow(davg)) davg %>% arrange(desc(Value)) %>% pull(class) else unique(dplot$class)
...
for (fam in FAMILIES) {
  ...
  family_label <- if (str_detect(fam, "celltype")) {
    ...
    "Per-Class Metrics — GenotypeNEXT"
  } else {
  ...
  # Five separate plots (per metric): folds 0..5 + Avg(1–5)
  for (mn in METRICS) {
