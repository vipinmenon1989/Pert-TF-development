...
def _auto_budget(config, n_obs_hint=None, logger=None):
    """
    Derive safe defaults from config + dataset size. One API, no per-run edits.
    Respects env overrides so you can force behavior from the job script.
    """
    ...
    # env override wins; else scale with sequence length
    default_every = 2 if L <= 4096 else 5
    ...
    # --- UMAP subsample budget ---
    # env override wins; else scale with dataset size and sequence length
    try:
    ...
    if n_obs is None:
        ...
        cap = 20000 if L <= 4096 else 12000
    else:
        # keep ~10–20% up to a cap, shrink if long seq
        frac_cap = max(5000, min(30000, int(0.2 * n_obs)))
        cap = frac_cap if L <= 4096 else max(4000, int(frac_cap * 0.6))
    ...
    # --- neighbors params (cheaper when big/long) ---
    base_k = 10 if (L > 4096 or (n_obs and n_obs > 100000)) else 15
    min_dist = 0.8 if (L > 4096 or (n_obs and n_obs > 100000)) else 0.5
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.no_grad():
            if not masked_positions.any():
                logger.warning("Batch has no masked positions; skipping masked_relative_error for this batch.")
                skip_mre = True
                mre = None
                mre_next = None
            else:
                ...
                mre_next = masked_relative_error(output_dict["mlm_output"], target_values_next, masked_positions)
...
def eval_testdata(
    ...
    if adata_t.n_obs == 0:
        if logger is not None:
            logger.warning(f"[eval_testdata:{eval_key}] No cells after filtering to known cell types; skipping metrics.")
...
def wrapper_train(model, config, data_gen,
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for sample in train_loader:
            ...
            masked_positions = input_values.eq(config.mask_value)
            if not masked_positions.any():
                logger.warning("SANITY CHECK: No masked positions in sample batch. Masking may not be applied.")
            else:
                logger.info("SANITY CHECK: Masking present in sample batch.")
            break
        ...
        if logger is not None:
            ...
            logger.info("-" * 89)
        wandb.log({
            f"validation/fold{fold}_epoch{epoch}/val_loss": val_loss,
            f"validation/fold{fold}_epoch{epoch}/mre": val_mre,
            f"validation/fold{fold}_epoch{epoch}/val_loss_next": val_loss_next,
            f"validation/fold{fold}_epoch{epoch}/mre_next": val_mre_next,
            f"validation/fold{fold}_epoch{epoch}/dab": val_dab,
            f"validation/fold{fold}_epoch{epoch}/cls": val_cls,
            f"validation/fold{fold}_epoch{epoch}/pert": val_pert,
            f"validation/fold{fold}_epoch{epoch}/ps": val_ps,
            "epoch": epoch})
        ...
        if epoch % 2 == 1:
            ...
            for eval_dict_key, eval_adata in eval_adata_dict.items():
                ...
                if not results or not isinstance(results, dict):
                    if logger is not None:
                        logger.warning(f"[wrapper_train:{eval_dict_key}] eval_testdata returned None/invalid; skipping logging for this split.")
                    continue
                
                results.setdefault("metrics",{})
                results.setdefault("data",None)
                ...
                for res_key, res_img_val in results.items():
                    if res_key in save_image_types:
                        ...
                        figure_logs[f"validation/fold{fold}/{res_key}_epoch{epoch}"]=wandb.Image(str(save_dir2/filename),caption=f"{res_key} | fold {fold} epoch {epoch}")
                if figure_logs:
